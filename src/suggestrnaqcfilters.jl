"""
    suggestrnaqcfilters(sums::Vector{Float64}, detected::Vector{Int32}, proportions::Dict{String,Vector{Float64}}; block = nothing, nummads = 3)
...
- `proportions`, a dictionary containing the proportion of counts in each feature subset.
...
- `thresholds`: a dictionary containing:
  ...
  - `proportions`: a dictionary where each key is a feature subset name and each value is a vector of doubles of length equal to the number of blocks, 
...
julia> scran.suggestrnaqcfilters(metrics["sums"], metrics["detected"], metrics["proportions"])
```
"""
function suggestrnaqcfilters(sums::Vector{Float64}, detected::Vector{Int32}, proportions::Dict{String,Vector{Float64}}; block = nothing, nummads = 3)
    ncells = length(sums)
    ...
    for (k, v) in proportions
        if length(v) != ncells
            throw(ErrorException("each entry of 'proportions' should be of the same length as 'sums'"))
        end
        push!(props, v)
        push!(subnames, k)
    end
